...
public sealed class PostgresOperationsStore(NpgsqlDataSource dataSource, Guid tenantId) : IOperationsStore
{
    ...
    public WorkOrder CreateWorkOrder(CreateWorkOrderInput input, string? actorName = null)
    {
        ...
        command.Parameters.AddWithValue(tenantId); command.Parameters.AddWithValue(number); command.Parameters.AddWithValue(input.Title.Trim()); command.Parameters.AddWithValue((object?)input.Description?.Trim() ?? DBNull.Value); command.Parameters.AddWithValue(input.AssetId == Guid.Empty ? DBNull.Value : input.AssetId); command.Parameters.AddWithValue(ToDatabase(input.Priority)); command.Parameters.AddWithValue((object?)input.AssignedTo?.Trim() ?? DBNull.Value); command.Parameters.AddWithValue((object?)input.DueAt ?? DBNull.Value);
        using var reader = command.ExecuteReader(); reader.Read();
